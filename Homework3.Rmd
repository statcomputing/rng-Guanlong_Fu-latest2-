...
##  Q1  1)
$$
Q(\Psi|\Psi^{(k)})= E_{z}\{l_{n}^{c}(\Psi\}\;=\sum_{i=1}^{n}\sum_{j=1}^{m}p_{ij}^{(k+1)}(z_{ij}|y_{i},x_{i};\Psi^{k})lnp(z_{i}=k,y_{i},x_{i}|\Psi)\;
$$
$$where,p_{ij}^{(k+1)}(z_{ij}|y_{i},x_{i};\Psi^{k})=\frac{p(z_{i}=k,y_{i},x_{i}|\Psi^{k})}{\sum_{j=1}^{m}p(z_{j}=s,y_{i},x_{i}|\Psi^{k})}= \frac{\pi_{j}^{k}\phi(y_{i}-x_{i}^{T}\beta_{j}^{k};0,\sigma^{2})}{\sum_{j=1}^{m}\pi_{j}^{k}\phi(y_{i}-x_{i}^{T}\beta_{j}^{k};0,\sigma^{2})},\;and\;lnp(z_{i}=k,y_{i},x_{i}|\Psi)=log\pi_{j}+log\phi(y_{i}-x_{i}^{T}\beta_{j}^{k};0,\sigma^{2}) $$
2.a)
 
the part of$Q(\Psi|\Psi^{(k)})$contains$pi_{j}^{k+1}$is$\sum_{i=1}^{n}\sum_{j=1}^{m}p_{ij}^{(k+1)}(z_{ij}|y_{i},x_{i};\Psi^{k})ln(2\pi)^{-d/2}\pi_{ij}^{k+1}$ 
and itis suffice to min 
$$\sum_{i=1}^{n}(\sum_{j=1}^{m}p_{ij}^{(k+1)}(z_{ij}|y_{i},x_{i};\Psi^{k}))\pi_{ij}^{k+1}\;subject\;to\sum_{j=1}^{m}\pi_{ij}^{k+1}=1$$
then we get 2).a)

2b)
the only part of$Q(\Psi|\Psi^{(k)})$contains$\beta_{j}^{k+1}$is
$$
\sum_{i=1}^{n}p_{ij}^{(k+1)}(y_{i}-x_{i}^{T}\beta_{j})^{'}\Sigma_{k}^{-1}(y_{i}-x_{i}^{T}\beta_{j})
$$
minimize it we find 2.b)

2.c)
The only part of $Q(\Psi|\Psi^{(k)})$contains$\sigma^{2(k+1)}$is
$$
\sum_{i=1}^{n}ln|\Sigma_{k}|+\sum_{i=1}^{n}p_{ij}^{(k+1)}(y_{i}-x_{i}^{T}\beta_{j})^{'}\Sigma_{k}^{-1}(y_{i}-x_{i}^{T}\beta_{j})
$$
minimize this part we can get 2 c)

# Qestion 2 
a) 
$$
\int_{0}^{\infty}[2x^{\theta-1}e^{-x}+x^{\theta-\frac{1}{2}}e^{-x}]dx=\frac{1}{C},\;we\;have\;C=\frac{1}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}
$$
and g is a mixture gamma with pdf:
$$
\frac{2\Gamma(\theta)}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}\frac{1}{\Gamma(\theta)}x^{\theta-1}e^{-x}+\frac{\Gamma(\theta+\frac{1}{2})}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}\frac{1}{\Gamma(\theta+\frac{1}{2})}x^{\theta+\frac{1}{2}-1}e^{-x}
$$
2b)

alg: first sample n integers from 1 and 2 acorrding to the probability$\frac{2\Gamma(\theta)}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}$and the probability$\frac{\Gamma(\theta+\frac{1}{2})}{2\Gamma(\theta)+\Gamma(\theta+\frac{1}{2})}$, and then sample n observations from the 1th or 2nd gamma distribution acorrding to vector of sampled integer 1s and 2s

x1 is the sampled mixture gamma distribution
...
use density() to estimate the density from the sample and use real density function of g to calculate the true density from a generated x interval and then plot
...
first step: # write a function to calculate the value of q(x)/g(x)
...
second step: find the optimal value of alpha - the inverse of the accpetance rate

```{r,warning=FALSE}
a <- -nlminb(0.2,alfa, theta = 1)$objective
```

Third step: rejection sampling
...
I used $\frac{1}{2}\beta(a,1)+\frac{1}{2}\beta(1,b)$ as the mixture beta to be the instrumental distribution

first write a function to sample from mixture of beta
...
second write a function to calcualte q(x)/g(x) and a function to find value of alpha
...
Third to implement rejection sampling from f(x) with g(x) as the instrument function
```{r}
i <- 1
...
first write two functions calcualte g1(x) and g2(x), which are two beta distributions
...
Second  # calculate the value of alpha1 and alpha2 and assign the value to p proportionally tp alpha1 &2
...
Third rejection sampling: 
1) generate a random integer k from k= 1,2 according to pk proportion to alphak
2) sample xk from gk and U
3) use goes back to step 1 if U>qk/gk, o.w.return xk
and the first 100 of estimated density from the sample are showned
...
```{r}
 ...
 sample_d6 <- density(x89, n=10000)$y
 sample_d6[1:100]
```
